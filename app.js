...
for(const row of data["Next 3 Weeks"]||[])if(row.length<7)row.push("Not Started");
...
function save(record=true){const current=stateJSON();if(record&&current!==lastState){undoStack.push(lastState);if(undoStack.length>50)undoStack.shift();redoStack=[]}lastState=current;localStorage.setItem("financePlanDemo",JSON.stringify(data));localStorage.setItem("futureSheetsDemo",JSON.stringify([...futureSheets]));localStorage.setItem("dailyPlansDemo",JSON.stringify(dailyPlans));localStorage.setItem("pageMetaDemo",JSON.stringify(pageMeta));$("#saveState").textContent="All changes saved locally";updateHistoryButtons()}
function restoreState(serialized){const restored=JSON.parse(serialized);data=restored.data;futureSheets=new Set(restored.futureSheets);dailyPlans=restored.dailyPlans||{};pageMeta=restored.pageMeta||{};if(!data[active])active=Object.keys(data)[0];selectedRow=null;lastState=stateJSON();localStorage.setItem("financePlanDemo",JSON.stringify(data));localStorage.setItem("futureSheetsDemo",JSON.stringify([...futureSheets]));localStorage.setItem("dailyPlansDemo",JSON.stringify(dailyPlans));localStorage.setItem("pageMetaDemo",JSON.stringify(pageMeta));closeDrawer();closeTabMenu();tabs();render();renderCalendar();renderDashboard();updateHistoryButtons()}
...
function restoreDailyFromRow(row){const id=dailyIdFromRow(row);if(!id)return;const date=String(row[5]||"").slice(0,10)||dateKey(today),link=String(row[10]||"").match(/https?:\/\/[^\s·]+/)?.[0]||"",status=String(row[8]||"Not Started");(dailyPlans[date]??=[]).push({title:String(row[2]||""),details:String(row[3]||""),link,category:String(row[1]||""),owner:String(row[4]||""),status,completed:status==="Done",syncedTo:active,syncId:id});save();render();alert(`Todo restored to ${date}.`)}
function render(){
...
function localAnswer(msg){const rows=data[active],si=statusIndex(),done=rows.filter(r=>r[si]==="Done").length,wip=rows.filter(r=>r[si]==="WIP"),future=futureSheets.has(active);if(/risk|block|overdue/i.test(msg))return wip.length?`Potential attention items:\n${wip.map(r=>future?`• ${r[2]} — ${r[3]}`:`• ${r[2]} — due ${r[6]}; next: ${r[9]}`).join("\n")}`:"No open WIP items found in this sheet.";if(/priorit|next/i.test(msg))return future?rows.filter(r=>r[5]==="High").map(r=>`• ${r[2]} — ${r[3]}`).join("\n"):`Suggested focus:\n${wip.slice(0,3).map(r=>`• ${r[2]}: ${r[9]}`).join("\n")}`;return `${active} has ${rows.length} items: ${done} done and ${wip.length} in progress. ${wip.length?`Current focus is ${wip.map(r=>r[2]).join(", ")}.`:"No open work is recorded."}`}
function showAIProposal(action,sheet){const card=document.createElement("div"),args=action.arguments||{};card.className="ai-proposal";card.innerHTML=`<b>Proposed change</b><p>${esc(args.reason||"Please review this change.")}</p><div class="proposal-detail">${Object.entries(args).filter(([k])=>k!=="reason").map(([k,v])=>`<span><small>${esc(k.replaceAll("_"," "))}</small>${esc(v)}</span>`).join("")}</div><div><button class="proposal-reject">Dismiss</button><button class="proposal-apply">Apply change</button></div>`;card.querySelector(".proposal-reject").onclick=()=>card.remove();card.querySelector(".proposal-apply").onclick=()=>{if(action.type==="propose_task_update"){let found=null;for(const [name,rows] of Object.entries(data)){const i=headers(name).length,row=rows.find(r=>r[i]===args.task_id);if(row){found={name,row};break}}if(!found)return alert("This task no longer exists.");const col=headers(found.name).indexOf(args.field);if(col<0)return alert("This field is not available on the task.");found.row[col]=args.value}else if(action.type==="propose_new_task"){if(!data[sheet])return alert("The target page no longer exists.");const now=dateKey(new Date()),row=[data[sheet].length+1,args.category,args.topic,args.content,args.owner,now,args.due_date||now,now,args.status,args.next_action,"Created from an approved AI proposal",uid("task")];data[sheet].push(row)}save();render();card.classList.add("applied");card.innerHTML="✓ Change applied"};$("#chat").append(card);card.scrollIntoView({behavior:"smooth"})}
async function ask(message){addBubble(message,"user");const loading=addBubble("Thinking…","ai"),requestSheet=active;try{if(!$("#remoteToggle").checked){loading.textContent=localAnswer(message);return}const r=await fetch("/api/ai",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({message,plan:{sheet:active,headers:[...headers(),"_Task ID"],rows:data[active]}})});const out=await r.json();loading.textContent=r.ok?out.text:out.error==="AI_NOT_CONFIGURED"?"Connected AI is not configured yet. Add OPENAI_API_KEY on the server, or switch back to Local mode.":`AI error: ${out.error}`;if(r.ok)(out.actions||[]).forEach(action=>showAIProposal(action,requestSheet))}catch(e){loading.textContent="Could not reach the AI service. Your plan remains saved locally."}}
...
function todoStatus(todo){return todo.status||(todo.completed?"Done":"Not Started")}
function syncedWeeklyRow(todo){if(!todo.syncedTo||!data[todo.syncedTo])return null;return data[todo.syncedTo].find(item=>todo.syncId?String(item[10]||"").includes(todo.syncId):item[1]===todo.category&&item[2]===todo.title)||null}
function mirrorTodoStatus(todo){todo.completed=todoStatus(todo)==="Done";const row=syncedWeeklyRow(todo);if(row)row[statusIndex(todo.syncedTo)]=todoStatus(todo)}
function syncDailyStatusFromRow(row){const linked=findDailyById(dailyIdFromRow(row));if(!linked)return;linked.todo.status=String(row[statusIndex(active)]||"Not Started");linked.todo.completed=linked.todo.status==="Done"}
function renderTodos(){const todos=dailyPlans[selectedDate]||[],target=weeklyTargets().includes(active)?active:weeklyTargets()[0];$("#todoList").innerHTML=todos.length?todos.map((todo,i)=>{const missing=todo.syncedTo&&!weeklyTodoExists(todo),status=todoStatus(todo);return `<article class="todo-card ${statusClass(status)} ${status==="Done"?"completed":""} ${missing?"missing-sync":""}" data-edit-todo="${i}"><button class="todo-check ${status==="Done"?"done":""}" data-check-todo="${i}" title="Mark ${status==="Done"?"not completed":"completed"}">${status==="Done"?"✓":""}</button><div class="todo-card-body"><h3>${esc(todo.title)}</h3>${todo.details?`<p>${esc(todo.details)}</p>`:""}<div class="todo-meta"><span class="todo-status ${statusClass(status)}">${esc(status)}</span>${todo.category?`<span class="todo-category">${esc(todo.category)}</span>`:""}${todo.owner?`<small>${esc(todo.owner)}</small>`:""}${missing?`<span class="sync-missing">Removed from ${esc(todo.syncedTo)}</span>`:todo.syncedTo?`<span class="synced">✓ Synced to ${esc(todo.syncedTo)}</span>`:`<button data-sync-todo="${i}">Sync to ${esc(target||"Weekly")}</button>`}</div></div></article>`}).join(""):`<div class="todo-empty">No plans yet.<br>Select “Add Todo” to plan this day.</div>`;document.querySelectorAll("[data-edit-todo]").forEach(card=>card.onclick=()=>openTodo(+card.dataset.editTodo));document.querySelectorAll("[data-sync-todo]").forEach(b=>b.onclick=e=>{e.stopPropagation();prepareWeeklySync(+b.dataset.syncTodo,target)})}
new MutationObserver(()=>{document.querySelectorAll(".todo-card:not([data-link-ready])").forEach(card=>{card.dataset.linkReady="true";const todo=(dailyPlans[selectedDate]||[])[+card.dataset.editTodo],url=safeUrl(todo?.link);if(!url)return;const a=document.createElement("a");a.className="todo-link";a.href=url;a.target="_blank";a.rel="noopener noreferrer";a.textContent="↗ Open link";a.onclick=e=>e.stopPropagation();card.querySelector(".todo-card-body")?.insertBefore(a,card.querySelector(".todo-meta"))})}).observe($("#todoList"),{childList:true,subtree:true});
new MutationObserver(()=>{document.querySelectorAll(".todo-card.missing-sync:not([data-syncback-ready])").forEach(card=>{card.dataset.syncbackReady="true";const index=+card.dataset.editTodo,todo=(dailyPlans[selectedDate]||[])[index];if(!todo)return;const button=document.createElement("button");button.type="button";button.className="sync-back";button.textContent="↻ Sync Back";button.onclick=e=>{e.stopPropagation();prepareWeeklySync(index,todo.syncedTo||weeklyTargets()[0])};card.querySelector(".todo-meta")?.append(button)})}).observe($("#todoList"),{childList:true,subtree:true});
...
new MutationObserver(()=>{document.querySelectorAll(".todo-card:not(.missing-sync):not([data-weekly-link-ready])").forEach(card=>{card.dataset.weeklyLinkReady="true";const todo=(dailyPlans[selectedDate]||[])[+card.dataset.editTodo];if(!todo?.syncedTo)return;const button=document.createElement("button");button.type="button";button.className="view-weekly";button.textContent="View in Weekly →";button.onclick=e=>{e.stopPropagation();goToWeeklyTodo(todo)};card.querySelector(".todo-meta")?.append(button)})}).observe($("#todoList"),{childList:true,subtree:true});
$("#todoList").addEventListener("click",e=>{const button=e.target.closest("[data-check-todo]");if(!button)return;e.preventDefault();e.stopImmediatePropagation();const todo=(dailyPlans[selectedDate]||[])[+button.dataset.checkTodo];if(!todo)return;if(todoStatus(todo)==="Done")todo.status=todo.statusBeforeDone||"WIP";else{todo.statusBeforeDone=todoStatus(todo);todo.status="Done"}mirrorTodoStatus(todo);save();renderCalendar()},{capture:true});
$("#rowForm").addEventListener("submit",()=>{if(!pendingTodoSync)return;const todo=dailyPlans[pendingTodoSync.date]?.[pendingTodoSync.index];if(!todo?.completed)return;const statusField=$("#rowForm").elements[`f${statusIndex()}`];if(statusField)statusField.value="Done"},{capture:true});
function prepareWeeklySync(index,target){if(!target)return alert("Create a Weekly Page first.");const todo=dailyPlans[selectedDate][index],id=todo.syncId||`${Date.now()}-${Math.random().toString(36).slice(2,8)}`;pendingTodoSync={date:selectedDate,index,target,id};$("#todoDialog").close();active=target;showCalendar(false);tabs();render();openDrawer(null);const link=todo.link?` · ${todo.link}`:"",values={Category:todo.category,Topic:todo.title,Content:todo.details,"Focal Name":todo.owner,Date:selectedDate,"Due Date":selectedDate,"Last Update":selectedDate,Status:todoStatus(todo),Note:`Synced from Daily Plan · ${selectedDate}${link} · [Daily:${id}]`};for(const [label,value] of Object.entries(values)){const i=headers().indexOf(label);if(i<0)continue;if(label==="Category"){const hidden=$("#categoryValue");if(hidden){hidden.value=value||"";$("#categoryTrigger span").textContent=value||"Select a category…"}}else{const input=$("#rowForm").elements[`f${i}`];if(input)input.value=value||""}}}
function openTodo(index=null){todoEditingIndex=index;const todo=index===null?null:(dailyPlans[selectedDate]||[])[index];$("#todoForm").reset();$("#todoDialogTitle").textContent=todo?"Edit Todo":"Add Todo";$("#saveTodo").textContent=todo?"Save Changes":"Add Todo";$("#deleteTodo").style.display=todo?"block":"none";const form=$("#todoForm").elements;form.status.value=todo?todoStatus(todo):"Not Started";if(todo){form.title.value=todo.title;form.details.value=todo.details||"";form.link.value=todo.link||""}$("#todoDialog").showModal()}
$("#calendarBtn").onclick=()=>showCalendar(true);$("#weeklyBtn").onclick=$("#backToWeekly").onclick=()=>showCalendar(false);$("#refreshBtn").onclick=()=>location.reload();$("#prevMonth").onclick=()=>{calendarMonth=new Date(calendarMonth.getFullYear(),calendarMonth.getMonth()-1,1);renderCalendar()};$("#nextMonth").onclick=()=>{calendarMonth=new Date(calendarMonth.getFullYear(),calendarMonth.getMonth()+1,1);renderCalendar()};$("#todayBtn").onclick=()=>{selectedDate=dateKey(today);calendarMonth=new Date(today.getFullYear(),today.getMonth(),1);renderCalendar()};$("#addTodo").onclick=()=>openTodo(null);$("#closeTodo").onclick=$("#cancelTodo").onclick=()=>$("#todoDialog").close();$("#deleteTodo").onclick=()=>{if(todoEditingIndex===null)return;if(confirm("Delete this Todo?")){dailyPlans[selectedDate].splice(todoEditingIndex,1);if(!dailyPlans[selectedDate].length)delete dailyPlans[selectedDate];save();$("#todoDialog").close();renderCalendar()}};
...
$("#todoForm").onsubmit=e=>{e.preventDefault();const form=new FormData(e.currentTarget),previous=todoEditingIndex===null?null:dailyPlans[selectedDate][todoEditingIndex],link=String(form.get("link")||"").trim(),status=String(form.get("status")||"Not Started");if(link&&!safeUrl(link))return alert("Please enter a valid http:// or https:// link.");const todo={title:String(form.get("title")),details:String(form.get("details")||""),link,category:previous?.category||"",owner:previous?.owner||"",status,completed:status==="Done",syncedTo:previous?.syncedTo||null,syncId:previous?.syncId||null,statusBeforeDone:previous?.statusBeforeDone||"WIP"};mirrorTodoStatus(todo);if(todoEditingIndex===null)(dailyPlans[selectedDate]??=[]).push(todo);else dailyPlans[selectedDate][todoEditingIndex]=todo;save();$("#todoDialog").close();renderCalendar()};
...
function renderDashboard(){const sheets=dashboardSheets();if(!sheets.length)return;const select=$("#dashboardWeek"),current=select.value&&data[select.value]?select.value:(sheets.includes(active)?active:sheets[0]);select.innerHTML=sheets.map(name=>`<option ${name===current?"selected":""}>${esc(name)}</option>`).join("");$("#dashboardPrintMeta").textContent=`${current} · Exported ${new Date().toLocaleDateString("en-US")}`;const rows=data[current],si=statusIndex(current),done=rows.filter(r=>r[si]==="Done").length,wip=rows.filter(r=>r[si]==="WIP").length,notStarted=rows.length-done-wip,completion=rows.length?Math.round(done/rows.length*100):0,now=dateKey(today),overdue=rows.filter(r=>r[6]&&String(r[6]).slice(0,10)<now&&r[si]!=="Done");$("#dashboardCards").innerHTML=[["TOTAL ITEMS",rows.length,""],["COMPLETION",`${completion}%`,"success"],["DONE",done,"success"],["IN PROGRESS",wip,""],["OVERDUE",overdue.length,"danger"]].map(([label,value,cls])=>`<article class="dash-card ${cls}"><span>${label}</span><b>${value}</b></article>`).join("");const donePct=rows.length?done/rows.length*100:0,wipPct=rows.length?wip/rows.length*100:0;$("#statusOverview").innerHTML=`<div class="status-layout"><div class="status-donut" style="background:conic-gradient(#178b50 0 ${donePct}%,#e5a400 ${donePct}% ${donePct+wipPct}%,#1769c2 ${donePct+wipPct}% 100%)"></div><div class="status-legend">${[["Done",done,"#178b50"],["WIP",wip,"#e5a400"],["Not Started",notStarted,"#1769c2"]].map(x=>`<div class="legend-row"><i style="background:${x[2]}"></i><span>${x[0]}</span><b>${x[1]}</b></div>`).join("")}</div></div>`;const counts={};rows.forEach(r=>counts[r[1]||"Uncategorized"]=(counts[r[1]||"Uncategorized"]||0)+1);const max=Math.max(1,...Object.values(counts));$("#categoryBreakdown").innerHTML=Object.entries(counts).sort((a,b)=>b[1]-a[1]).map(([name,count])=>`<div class="category-row"><div class="category-label"><span>${esc(name)}</span><b>${count}</b></div><div class="category-bar"><i style="width:${count/max*100}%"></i></div></div>`).join("")||'<div class="empty-state">No categories yet</div>';const attention=[...overdue.map(r=>({r,label:"Overdue"})),...rows.filter(r=>r[si]==="WIP"&&!overdue.includes(r)).map(r=>({r,label:"WIP"}))].slice(0,6);$("#attentionList").innerHTML=attention.length?attention.map(x=>`<div class="attention-item"><i></i><div><b>${esc(x.r[2])}</b><br><small>${esc(x.r[1])}</small></div><small>${x.label}</small></div>`).join(""):'<div class="empty-state">Nothing needs attention</div>';const dates=rows.map(r=>String(r[5]||"").slice(0,10)).filter(Boolean).sort(),start=dates[0]||"0000",end=dates.at(-1)||"9999",todos=Object.entries(dailyPlans).filter(([date])=>date>=start&&date<=end).flatMap(([,items])=>items),synced=todos.filter(t=>t.syncedTo===current&&weeklyTodoExists(t)).length,broken=todos.filter(t=>t.syncedTo===current&&!weeklyTodoExists(t)).length;$("#dailyHealth").innerHTML=`<div class="health-row"><span>Daily Todos</span><b>${todos.length}</b></div><div class="health-row good"><span>Completed Todos</span><b>${todos.filter(t=>t.completed).length}</b></div><div class="health-row good"><span>Synced to this Week</span><b>${synced}</b></div><div class="health-row ${broken?"warn":"good"}"><span>Broken Sync Links</span><b>${broken}</b></div>`;const index=sheets.indexOf(current),previous=index>0?data[sheets[index-1]]:null,previousRate=previous?.length?Math.round(previous.filter(r=>r[statusIndex(sheets[index-1])]==="Done").length/previous.length*100):null;$("#weekComparison").textContent=previousRate===null?"No previous week":"Previous week: "+previousRate+"% · "+(completion-previousRate>=0?"+":"")+(completion-previousRate)+" pts"}
$("#dashboardBtn").onclick=showDashboard;$("#dashboardRefresh").onclick=renderDashboard;$("#dashboardWeek").onchange=renderDashboard;$("#dashboardExportPdf").onclick=()=>{renderDashboard();document.body.classList.add("dashboard-printing");const previousTitle=document.title,week=$("#dashboardWeek").value;document.title=`Weekly Dashboard - ${week}`;const cleanup=()=>{document.body.classList.remove("dashboard-printing");document.title=previousTitle};addEventListener("afterprint",cleanup,{once:true});setTimeout(()=>window.print(),50)};
