...
function mirrorTodoCompletion(todo){if(!todo.syncedTo||!data[todo.syncedTo])return;const row=data[todo.syncedTo].find(item=>todo.syncId?String(item[10]||"").includes(todo.syncId):item[1]===todo.category&&item[2]===todo.title);if(!row)return;if(todo.completed){if(row[8]!=="Done")todo.weeklyStatusBeforeDone=row[8]||"Not Started";row[8]="Done"}else if(row[8]==="Done")row[8]=todo.weeklyStatusBeforeDone||"WIP"}
function renderTodos(){const todos=dailyPlans[selectedDate]||[],target=weeklyTargets().includes(active)?active:weeklyTargets()[0];$("#todoList").innerHTML=todos.length?todos.map((todo,i)=>{const missing=todo.syncedTo&&!weeklyTodoExists(todo);return `<article class="todo-card ${todo.completed?"completed":""} ${missing?"missing-sync":""}" data-edit-todo="${i}"><button class="todo-check ${todo.completed?"done":""}" data-check-todo="${i}" title="Mark ${todo.completed?"not completed":"completed"}">${todo.completed?"✓":""}</button><div class="todo-card-body"><h3>${esc(todo.title)}</h3>${todo.details?`<p>${esc(todo.details)}</p>`:""}<div class="todo-meta">${todo.category?`<span class="todo-category">${esc(todo.category)}</span>`:""}${todo.owner?`<small>${esc(todo.owner)}</small>`:""}${missing?`<span class="sync-missing">Removed from ${esc(todo.syncedTo)}</span>`:todo.syncedTo?`<span class="synced">✓ Synced to ${esc(todo.syncedTo)}</span>`:`<button data-sync-todo="${i}">Sync to ${esc(target||"Weekly")}</button>`}</div></div></article>`}).join(""):`<div class="todo-empty">No plans yet.<br>Select “Add Todo” to plan this day.</div>`;document.querySelectorAll("[data-edit-todo]").forEach(card=>card.onclick=()=>openTodo(+card.dataset.editTodo));document.querySelectorAll("[data-check-todo]").forEach(b=>b.onclick=e=>{e.stopPropagation();const todo=todos[+b.dataset.checkTodo];todo.completed=!todo.completed;save();renderCalendar()});document.querySelectorAll("[data-sync-todo]").forEach(b=>b.onclick=e=>{e.stopPropagation();prepareWeeklySync(+b.dataset.syncTodo,target)})}
...
$("#todoForm").onsubmit=e=>{e.preventDefault();const form=new FormData(e.currentTarget),previous=todoEditingIndex===null?null:dailyPlans[selectedDate][todoEditingIndex],link=String(form.get("link")||"").trim();if(link&&!safeUrl(link))return alert("Please enter a valid http:// or https:// link.");const todo={title:String(form.get("title")),details:String(form.get("details")||""),link,category:previous?.category||"",owner:previous?.owner||"",completed:previous?.completed||false,syncedTo:previous?.syncedTo||null,syncId:previous?.syncId||null};if(todoEditingIndex===null)(dailyPlans[selectedDate]??=[]).push(todo);else dailyPlans[selectedDate][todoEditingIndex]=todo;save();$("#todoDialog").close();renderCalendar()};
