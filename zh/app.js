...
function badge(value){const c={"已完成":"done","进行中":"wip","未开始":"not-started","High":"high","Medium":"medium"}[String(value)];return c?`<span class="badge ${c}">${value}</span>`:value}
...
function restoreDailyFromRow(row){const id=dailyIdFromRow(row);if(!id)return;const date=String(row[5]||"").slice(0,10)||dateKey(today),link=String(row[10]||"").match(/https?:\/\/[^\s·]+/)?.[0]||"",status=String(row[8]||"未开始");(dailyPlans[date]??=[]).push({title:String(row[2]||""),details:String(row[3]||""),link,category:String(row[1]||""),owner:String(row[4]||""),status,completed:status==="已完成",syncedTo:active,syncId:id});save();render();alert(`Todo restored to ${date}.`)}
function render(){
...
function statusClass(value){return {"已完成":"status-done","进行中":"status-wip","未开始":"status-not-started"}[String(value)]||""}
...
function todoStatus(todo){return todo.status||(todo.completed?"已完成":"未开始")}
...
function mirrorTodoStatus(todo){todo.completed=todoStatus(todo)==="已完成";const row=syncedWeeklyRow(todo);if(row)row[statusIndex(todo.syncedTo)]=todoStatus(todo)}
function syncDailyStatusFromRow(row){const linked=findDailyById(dailyIdFromRow(row));if(!linked)return;linked.todo.status=String(row[statusIndex(active)]||"未开始");linked.todo.completed=linked.todo.status==="已完成"}
function renderTodos(){const todos=dailyPlans[selectedDate]||[],target=weeklyTargets().includes(active)?active:weeklyTargets()[0];$("#todoList").innerHTML=todos.length?todos.map((todo,i)=>{const missing=todo.syncedTo&&!weeklyTodoExists(todo),status=todoStatus(todo);return `<article class="todo-card ${statusClass(status)} ${status==="已完成"?"completed":""} ${missing?"missing-sync":""}" data-edit-todo="${i}"><button class="todo-check ${status==="已完成"?"done":""}" data-check-todo="${i}" title="Mark ${status==="已完成"?"not completed":"completed"}">${status==="已完成"?"✓":""}</button><div class="todo-card-body"><h3>${esc(todo.title)}</h3>${todo.details?`<p>${esc(todo.details)}</p>`:""}<div class="todo-meta"><span class="todo-status ${statusClass(status)}">${esc(status)}</span>${todo.category?`<span class="todo-category">${esc(todo.category)}</span>`:""}${todo.owner?`<small>${esc(todo.owner)}</small>`:""}${missing?`<span class="sync-missing">已从以下页面删除：${esc(todo.syncedTo)}</span>`:todo.syncedTo?`<span class="synced">✓ 已同步到${esc(todo.syncedTo)}</span>`:`<button data-sync-todo="${i}">同步到${esc(target||"Weekly")}</button>`}</div></div></article>`}).join(""):`<div class="todo-empty">暂无计划。<br>点击“添加待办”开始安排。</div>`;document.querySelectorAll("[data-edit-todo]").forEach(card=>card.onclick=()=>openTodo(+card.dataset.editTodo));document.querySelectorAll("[data-sync-todo]").forEach(b=>b.onclick=e=>{e.stopPropagation();prepareWeeklySync(+b.dataset.syncTodo,target)})}
...
$("#todoList").addEventListener("click",e=>{const button=e.target.closest("[data-check-todo]");if(!button)return;e.preventDefault();e.stopImmediatePropagation();const todo=(dailyPlans[selectedDate]||[])[+button.dataset.checkTodo];if(!todo)return;if(todoStatus(todo)==="已完成")todo.status=todo.statusBeforeDone||"进行中";else{todo.statusBeforeDone=todoStatus(todo);todo.status="已完成"}mirrorTodoStatus(todo);save();renderCalendar()},{capture:true});
...
$("#todoForm").onsubmit=e=>{e.preventDefault();const form=new FormData(e.currentTarget),previous=todoEditingIndex===null?null:dailyPlans[selectedDate][todoEditingIndex],link=String(form.get("link")||"").trim(),status=String(form.get("status")||"未开始");if(link&&!safeUrl(link))return alert("请输入有效的 http:// 或 https:// 链接。");const todo={title:String(form.get("title")),details:String(form.get("details")||""),link,category:previous?.category||"",owner:previous?.owner||"",status,completed:status==="已完成",syncedTo:previous?.syncedTo||null,syncId:previous?.syncId||null,statusBeforeDone:previous?.statusBeforeDone||"进行中"};mirrorTodoStatus(todo);if(todoEditingIndex===null)(dailyPlans[selectedDate]??=[]).push(todo);else dailyPlans[selectedDate][todoEditingIndex]=todo;save();$("#todoDialog").close();renderCalendar()};
